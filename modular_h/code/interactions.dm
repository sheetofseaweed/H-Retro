...
mob/living/carbon/human/proc/cum(mob/living/carbon/human/H as mob, mob/living/carbon/human/P as mob, var/hole = "floor")
	...
	if (H.gender == MALE)
		...
		playsound(loc, "modular_h/sound/interactions/final_m[rand(1, 5)].ogg", 70, 1, 0)
		...
		if (istype(P.loc, /obj/structure/closet))
			P.visible_message("<B>[H] [message].</B>")
		playsound(loc, "modular_h/sound/interactions/final_f[rand(1, 3)].ogg", 90, 1, 0)
	...
	switch(hole)
		...
		if("fingering")
			...
			if (P.stat != DEAD && P.stat != UNCONSCIOUS)
				...
				if (P.lust >= P.resistenza)
					...
					P.sexmoan()
			playsound(loc, "modular_h/sound/interactions/champ_fingering.ogg", 50, 1, -1)
			H.do_fucking_animation(P)
		...
		if("blowjob")
			...
			if (P.stat != DEAD && P.stat != UNCONSCIOUS)
				...
				if (P.lust >= P.resistenza)
					P.cum(P, H, "mouth")
			playsound(loc, "modular_h/sound/interactions/bj[rand(1, 11)].ogg", 50, 1, -1)
			H.do_fucking_animation(P)
			if (prob(P.potenzia))
		...
		if("vaginal")
			...
			if (P.stat != DEAD && P.stat != UNCONSCIOUS)
				...
				if (P.lust >= P.resistenza)
					...
					P.sexmoan()
			H.do_fucking_animation(P)
			playsound(loc, "modular_h/sound/interactions/bang[rand(1, 3)].ogg", 70, 1, -1)
		...
		if("oral")
			...
			H.do_fucking_animation(P)
			playsound(loc, "modular_h/sound/interactions/oral[rand(1, 2)].ogg", 70, 1, -1)
			if (P.species.name == "Slime People")
				playsound(loc, "modular_h/sound/interactions/champ[rand(1, 2)].ogg", 50, 1, -1)
			if (prob(H.potenzia))
		...
		if ("Human")
			if (prob(H.lust / H.resistenza * 65))
				...
				if(!istype(loc, /obj/structure/closet))
					playsound(loc, "modular_h/sound/interactions/moan_[g][sexmoan].ogg", 70, 1, 1)
				else if (g == "f")
					playsound(loc, "modular_h/sound/interactions/under_moan_f[rand(1, 4)].ogg", 70, 1, 1)
				lastmoan = sexmoan
